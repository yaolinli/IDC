...
class Model(nn.Module):
    def __init__(self, ff_dim, img_embs, n_hidden, n_head, n_block, img_enc_block, vocab_size, dropout, max_len, CLS):
        ...
        self.word_embedding = nn.Embedding(vocab_size, n_hidden)
        ...
        self.pooler = nn.Sequential(nn.Linear(self.n_hidden*2, 2))
    ...
    def forward(self, batch, compute_loss=True):
        ...
        masked_lm_loss = F.cross_entropy(prediction_scores,
                                        ...
                                        reduction='none')  
        '''
        compute distractor classification(0/1) loss
        '''
        CLS = torch.cat((output[:, 0, :], output[:, img_toklen, :]), dim=1)
        # feed the cls feature to 2 layer FC layers
        itm_scores = self.pooler(CLS)
        align_label = Variable(batch['diff_label']).cuda()
        align_label = align_label.view(-1)
        itm_loss = F.cross_entropy(itm_scores, align_label, reduction='none')
        if compute_loss:
            return masked_lm_loss, itm_loss
        else:
            return prediction_scores, itm_scores
