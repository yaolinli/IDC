...
class Model(nn.Module):
    def __init__(self, ff_dim, img_embs, n_hidden, n_head, n_block, se_block, de_block, vocab_size, dropout, max_len, tasks, CLS):
        ...
        self.classifier = nn.Sequential(nn.Linear(self.n_hidden, self.num_classes))
        self.aligner = nn.Sequential(nn.Linear(self.n_hidden*2, 2))
        self.feat_regress = RegionFeatureRegression(n_hidden, img_embs, self.img_project[0].weight)

    def forward(self, batch, task, compute_loss=True):
        if task == 'nabirds':
            return self.nabirds_forward(batch, compute_loss)
        else:
            return self.tasks_forward(batch, task, compute_loss)
 
    def nabirds_forward(self, batch, compute_loss=True):
        ...
        img_toklen = Img1.size(1)
        # add two <CLS> token
        CLS = torch.tensor([[self.CLS]])
        CLS = CLS.repeat(Img1.size(0), 1)
        CLS = self.word_embedding(CLS.cuda())
        CLS1, CLS2 = self.position_encoding(CLS, mode='cls', pos=1), self.position_encoding(CLS, mode='cls', pos=3)
        ...
        # '''classification loss'''
        single_cls1 = image_embs1[:, 0]
        single_cls2 = image_embs2[:, 0]
        pred_logits1 = self.classifier(single_cls1)
        pred_logits2 = self.classifier(single_cls2)
        class_label1 = Variable(batch['class1']).cuda().view(-1)
        class_label2 = Variable(batch['class2']).cuda().view(-1)
        class_logits = torch.cat((pred_logits1, pred_logits2), dim=0)
        class_labels = torch.cat((class_label1, class_label2), dim=0)
        class_loss = F.cross_entropy(class_logits, class_labels, reduction='mean')
        # '''matching loss'''
        out_cls1 = image_embs[:, 0]
        out_cls2 = image_embs[:, 1+img_toklen]
        g_cls = torch.cat((out_cls1, out_cls2), dim=1)
        match_logits = self.aligner(g_cls) # feed the cls feature to 2 layer FC layers
        align_labels = Variable(batch['label']).cuda().view(-1)
        match_loss =  F.cross_entropy(match_logits, align_labels, reduction='mean')
        # '''constractive loss'''
        single_cls = torch.cat((single_cls1, single_cls2), dim=0)
        contrast_loss = con_loss(single_cls, class_labels)
        if compute_loss:
            return class_loss+match_loss+contrast_loss, class_logits
        else:
            return contrast_loss, class_logits, class_labels, match_logits, align_labels    
 
    def tasks_forward(self, batch, task, compute_loss=True):
        batch = defaultdict(lambda: None, batch)
        Img1 = Variable(batch['img1']).cuda()
        Img2 = Variable(batch['img2']).cuda()
        ...
        # concate [cls1, img1, cls2, img2] as input to image encoder
        image_embs1 = torch.cat((CLS1, img_embs1), dim=1)
        image_embs2 = torch.cat((CLS2, img_embs2), dim=1)
        image_embs1 = self.single_img_encoder(image_embs1)
        image_embs2 = self.single_img_encoder(image_embs2)
    ...
    def mvm_forward(self, task, input_embs, img_mask, img_label, img_toklen, start_idx, compute_loss=True):
        output = self.encoder(input_embs)
        img = torch.cat((output[:, start_idx['img1']:(start_idx['img1']+img_toklen), :], output[:, start_idx['img2']:(start_idx['img2']+img_toklen), :]), dim=1)
        # only compute masked tokens for better efficiency
